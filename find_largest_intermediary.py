...
def main(commands, output, expansion):
    ...
    with open(commands) as file:
        ...
        for part in data:
            ...
            res = int(
                str(subprocess.check_output(["wc", str(filename)]), "UTF-8")
                .strip()
                .split()[0]
            )
            ...
            if expansion:
                res = res // int(
                    str(subprocess.check_output(["wc", part["file"]]), "UTF-8")
                    .strip()
                    .split()[0]
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-e", action="store_true", help="Calculate size increase instead of total size."
    )
